...
def get_profile(battletag):
    ...
    response = requests.get("https://playoverwatch.com/en-gb/search/account-by-name/" + str({encoded_battletag}))
...
def get_stats(battletag, region):
    response = requests.get("https://owapi.net/api/v3/u/" + str({battletag}) + "/stats", headers=headers)
    if response.status_code != 200:
        return None
...
def get_heroes(battletag, region):
    response = requests.get("https://owapi.net/api/v3/u/" + str({battletag}) + "/heroes", headers=headers)
    if response.status_code != 200:
        return None
...
def get_blob(battletag, region):
    url = "https://owapi.net/api/v3/u/" + str({battletag}) + "/blob"
